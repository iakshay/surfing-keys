...
export const fakeSpot = (url = window.location.href) =>
  openLink(`https://fakespot.com/analyze?ra=true&url=${url}`, {
    newTab: true,
    active: false,
  })
...
export const viewGodoc = () =>
  openLink(`https://godoc.org/${util.getURLPath({ count: 2, domain: true })}`, {
    newTab: true,
  })
...
export const hn = {
  ...
  openLinkAndComments: (e) => {
    ...
    openLink(commentsUrl, { newTab: true })
    openLink(linkUrl, { newTab: true })
  },
}
...
export const wp = {
  ...
  viewWikiRank: () => {
    ...
    openLink(`https://wikirank.net/${lang}/${article}`, { newTab: true })
  },
...
// devdocs.io
export const dv = {
  scrollSidebar: (dir) => scrollElement(document.querySelector("._list"), dir),
  scrollContent: (dir) =>
    scrollElement(document.querySelector("._content"), dir),
}
