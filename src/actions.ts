...
actions.dg.goog = () => {
  const dg = actions.go.parseLocation()
...
actions.dg.perplexity = () => {
  const dg = actions.dg.parseLocation()
  const pp = new URL("https://www.perplexity.ai/search")
  if (dg.query) {
    pp.searchParams.set("q", dg.query)
  }
...
actions.gh.parseRepo = (url = window.location.href, rootOnly = false) => {
  ...
  const cond = ["github.com", "gist.github.com", "raw.githubusercontent.com"].includes(
    u.hostname,
  )
    && typeof user === "string"
    && user.length > 0
...
actions.gh.parseUser = (url = window.location.href, rootOnly = false) => {
  ...
  const isRoot = rest.length === 0
  const cond = u.origin === window.location.origin
    && typeof user === "string"
    && user.length > 0
